...
const BarChart = (props) => {
    ...
    useEffect(() => {
        ...
        for(const label in labels){
            datasets.push({
                label: labels[label],
                data: Array.apply(null, Array(24)).map(function () {}),
        ...
        for(const key in datasets){
            for(const value in props.values){
                if(datasets[key].label === props.values[value].deviceId){
                    const position = parseInt(props.values[value].hour.slice(0,2));
                    datasets[key].data[position] = parseInt(props.values[value].energyConsumption);
                }
            }
        }
